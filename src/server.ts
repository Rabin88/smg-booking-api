...
export function createApp(db: Database): Express {
  ...
  app.get("/availability", (req: Request, res: Response) => {
    ...
    const result = getAvailability(db, {
      ...
      formatId: format as string,
      cycleId: cycle as string
    });
  ...
  app.post("/holds", (req: Request, res: Response) => {
    const { campaignId, storeId, formatId, cycleId, quantity, traderId } =
      req.body ?? {};
    ...
    if (
      ...
      return res.status(400).json({
        error: {
          code: "missing_fields",
          message:
            "campaignId, storeId, formatId, cycleId, quantity and traderId are required"
        }
      });
    }
