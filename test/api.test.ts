...
beforeEach(() => {
  ...
  db.prepare("INSERT INTO store_format_capacity VALUES ('S412','aisle_barrier',2)").run();
  app = createApp(db);
});
...
describe('GET /availability', () => {
  test('returns the breakdown, not just a total', async () => {
    ...
    expect(res.body).toMatchObject({ capacity: 2, confirmed: 0, held: 0, available: 2 });
  });
...
describe('POST /holds', () => {
  ...
  test('409 when the space has gone, with the current availability', async () => {
    await request(app).post('/holds').send({ ...body, quantity: 2 });

    const res = await request(app).post('/holds').send({ ...body, campaignId: 'nestle' });
